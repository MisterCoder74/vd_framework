  ...
  render(){
    ...
    this.shadowRoot.innerHTML=`
      <style>
        ...
        button{background:${text};color:${bg};border:none;border-radius:4px;padding:6px 10px;cursor:pointer;}
        input[type="range"]{flex:1;}
      </style>
      <h4>${VDUtils.sanitizeHTML(title)}</h4>
      <video id="vid" src="${file}"></video>
      <div class="controls">
        <button id="play">▶</button>
        <input id="seek" type="range" min="0" max="100" value="0">
      </div>`;
    ...
    const playBtn=this.shadowRoot.querySelector("#play");
    const seek=this.shadowRoot.querySelector("#seek");
    this._addListener(playBtn,"click",()=>{if(video.paused){video.play();playBtn.textContent="⏸";}else{video.pause();playBtn.textContent="▶";}});
